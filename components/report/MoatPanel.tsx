...
const GRADE_STYLE: Record<string, { label: string; cls: string }> = {
  none: { label: "none", cls: "border-border bg-panel2 text-muted" },
  weak: { label: "weak", cls: "border-warn/40 bg-warn/10 text-warn" },
  plausible: { label: "plausible", cls: "border-accent2/40 bg-accent2/10 text-accent2" },
  strong: { label: "strong", cls: "border-good/40 bg-good/10 text-good" },
};
  ...
  return (
    <div>
      <div className="mb-2.5 flex items-baseline justify-between gap-2 font-mono text-sm uppercase tracking-[0.1em] text-muted">
        Defensibility
        <span className="text-[10px] normal-case tracking-normal">graded honest-to-zero — “none” is a finding, not a failure</span>
      </div>

      {moat.today && (
        <p className="mb-3 rounded-lg border border-border/70 bg-panel/40 p-3.5 text-sm leading-relaxed text-fg/90">
          <span className="font-semibold text-fg">What stops a copycat today: </span>
          {moat.today}
        </p>
      )}
      ...
      {paths.length > 0 && (
        <div className="overflow-hidden rounded-lg border border-border/70">
          {paths.map((p, i) => {
            const g = GRADE_STYLE[p.grade] ?? GRADE_STYLE.none;
            return (
              <div
                key={i}
                className={`flex flex-wrap items-baseline gap-x-3 gap-y-1 px-3.5 py-2 ${i % 2 ? "bg-panel/40" : "bg-panel/70"}`}
              >
                <span className="w-36 shrink-0 text-sm font-medium text-fg/90">{pretty(p.type)}</span>
                <span className={`rounded-full border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wide ${g.cls}`}>
                  {g.label}
                </span>
                {p.note && <span className="min-w-0 flex-1 text-xs leading-relaxed text-muted">{p.note}</span>}
              </div>
            );
          })}
        </div>
      ...
      {toBuild.length > 0 && (
        <div className="mt-3 rounded-lg border border-accent/30 bg-accent/5 p-3.5">
          <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-accent">
            What would have to become true to earn one
          </div>
          <ul className="mt-2 space-y-1.5">
            {toBuild.map((p, i) => (
              <li key={i} className="flex gap-2.5 text-sm leading-relaxed text-fg/90">
                <span className="mt-px shrink-0 font-mono text-xs text-accent" aria-hidden>→</span>
                <span>
                  {p.path && <b className="text-fg">{pretty(p.path)}: </b>}
                  {p.becomes_true}
                </span>
              </li>
            ))}
          ...
          <p className="mt-2 text-xs text-muted">
            These are targets for the wedge tournament / refine loop — position the idea so one of them starts becoming true.
          </p>
        </div>
      )}
    </div>
